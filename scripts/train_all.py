...
def train_scene(gpu, scene, factor):
    ...
    cmd = f"OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES={gpu} python train.py -s {dtu_path}/scan{scene} -m {output_dir}/scan{scene} -r 2 --depth_ratio 1 --lambda_dist 1000 --port 13{scene}"
    print(cmd)
    if not dry_run:
        os.system(cmd)
