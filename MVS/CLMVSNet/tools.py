...
def get_normal_grad_weight(img, aph=10):
    if len(img.shape) == 4:
        img = img.squeeze()
    _, hd, wd = img.shape  # 3 h w
    bottom_point = img[..., 2:hd,   1:wd-1]
    top_point    = img[..., 0:hd-2, 1:wd-1]
    right_point  = img[..., 1:hd-1, 2:wd]
    left_point   = img[..., 1:hd-1, 0:wd-2]

    grad_img_x = torch.acos((right_point * left_point).sum(dim=0, keepdim=True))#1 h w 
    grad_img_y = torch.acos((top_point * bottom_point).sum(dim=0, keepdim=True))#1 h w
    
    grad_img_x[grad_img_x!=grad_img_x] = 3.14
    grad_img_y[grad_img_y!=grad_img_y] = 3.14
    grad_img = torch.cat((grad_img_x, grad_img_y), dim=0) # 2 h w
    grad_img, _ = torch.max(grad_img, dim=0) # h w
    grad_img = torch.nn.functional.pad(grad_img[None,None], (1,1,1,1), mode='constant', value=3.14).squeeze()
    grad_img_mask = (grad_img * 180 / 3.14 < aph).float()#h w
    return grad_img_mask
