...
class RegressionDepth(nn.Module):
    ...
    def forward(self, cost_reg, depth_hypotheses, **kwargs):
        ...
        return {"depth": depth, "photometric_confidence": photometric_confidence, "prob_volume": prob_volume, "prob_volume_pre":prob_volume_pre,
...
class CasMVSNet(nn.Module):
    ...
    def forward(self, data, icc=False, scc=False, epoch=0):
        ...
        for stage_idx in range(self.num_stage):
            ...
            outputs.update({"mvs_features": features_stage})
            outputs.update({"ref_fea": features_stage[0]})
            outputs.update(outputs_stage)

        #为了测试多尺度feature加的代码
        #features_stage_0_ref = [feat["stage{}".format(1)] for feat in features][0]
        #features_stage_1_ref = [feat["stage{}".format(2)] for feat in features][0]
        #features_stage_2_ref = [feat["stage{}".format(3)] for feat in features][0]
        #h, w = features_stage_0_ref.shape[2:]
        #features_stage_0_ref = F.interpolate(features_stage_0_ref, scale_factor=4, mode="nearest")
        #features_stage_1_ref = F.interpolate(features_stage_1_ref, scale_factor=2, mode="nearest")
        #outputs.update({"ref_fea": features_stage_0_ref})
        return outputs
