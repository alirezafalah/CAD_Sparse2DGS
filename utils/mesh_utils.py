...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        color_list = []
        fea_list = []
        for view in self.viewpoint_stack:
            color_list.append(view.original_image.reshape(3, -1).permute(1, 0))
            fea_list.append(view.feature.reshape(8, -1).permute(1, 0))
        pre_color = torch.cat(color_list, dim=0)
        pre_fea = torch.cat(fea_list, dim=0)
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            render_pkg = self.render(viewpoint_cam, self.gaussians, fixed_color=torch.cat([pre_color, pre_fea], dim=-1))
